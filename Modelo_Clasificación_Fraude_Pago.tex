...
\usepackage{hyperref}
\hypersetup{
            pdftitle={Modelo\_Clasificación\_FraudeEnPago},
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Data}\OperatorTok{$}\NormalTok{C_New <-}\StringTok{ }\KeywordTok{ifelse}\NormalTok{(}\KeywordTok{is.na}\NormalTok{(Data}\OperatorTok{$}\NormalTok{C), }\KeywordTok{mean}\NormalTok{(Data}\OperatorTok{$}\NormalTok{C, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{), Data}\OperatorTok{$}\NormalTok{C)}
\KeywordTok{summary}\NormalTok{(Data}\OperatorTok{$}\NormalTok{C_New)}
\end{Highlighting}
\end{Shaded}
